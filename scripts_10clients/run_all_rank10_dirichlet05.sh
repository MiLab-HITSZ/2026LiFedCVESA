#!/usr/bin/env bash

set -eu

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
EPOCHS="${EPOCHS:-100}"
GAMA_WARMUP_EPOCHS="${GAMA_WARMUP_EPOCHS:-0}"
IID="${IID:-0}"
source "${SCRIPT_DIR}/common.sh"

RUN_GAMMA="${RUN_GAMMA:-1}"
RUN_NUM_STEAL="${RUN_NUM_STEAL:-1}"
RUN_POSITION="${RUN_POSITION:-1}"
GPU_LIST="${GPU_LIST:-0 1 2 3 4 5 6 7}"
JOBS_PER_GPU="${JOBS_PER_GPU:-1}"
LOG_DIR="${LOG_DIR:-${PROJECT_ROOT}/scripts_10clients/logs_rank10_dirichlet05}"
DRY_RUN="${DRY_RUN:-0}"

CONFIG_TAG="${CONFIG_TAG:-rank10_dirichlet05}"
MNIST_MODEL="${MNIST_MODEL:-cnn}"
FMNIST_MODEL="${FMNIST_MODEL:-cnn}"
CIFAR_MODEL="${CIFAR_MODEL:-resnet18_cifar}"
NONIID_MODE="${NONIID_MODE:-dirichlet}"
SHARDS_PER_USER="${SHARDS_PER_USER:-0}"
DIRICHLET_ALPHA="${DIRICHLET_ALPHA:-0.5}"
DIRICHLET_MIN_SIZE="${DIRICHLET_MIN_SIZE:-100}"

COMMON_OPT_ARGS=(
    --momentum=0.9
    --weight_decay=0.0005
    --lr_scheduler=cosine
    --min_lr=0.0001
    --noniid_mode="${NONIID_MODE}"
    --shards_per_user="${SHARDS_PER_USER}"
    --dirichlet_alpha="${DIRICHLET_ALPHA}"
    --dirichlet_min_size="${DIRICHLET_MIN_SIZE}"
)

CIFAR_DATA_ARGS=(
    --cifar_crop_size=32
    --cifar_normalize=1
)

JOBS_NAME=()
JOBS_ARGS=()
declare -A SEEN_JOB_ARGS=()

add_job() {
    job_name="$1"
    shift
    job_args="$*"
    if [ -n "${SEEN_JOB_ARGS[${job_args}]:-}" ]; then
        return 0
    fi
    SEEN_JOB_ARGS["${job_args}"]=1
    JOBS_NAME+=("${job_name}")
    JOBS_ARGS+=("${job_args}")
}

add_dataset_job() {
    job_name="$1"
    dataset="$2"
    model="$3"
    lr="$4"
    local_bs="$5"
    local_ep="$6"
    shift 6

    dataset_args=()
    if [ "${dataset}" = "cifar" ]; then
        dataset_args=("${CIFAR_DATA_ARGS[@]}")
    fi

    add_job "${job_name}" \
        --model="${model}" \
        --dataset="${dataset}" \
        --lr="${lr}" \
        --local_bs="${local_bs}" \
        --local_ep="${local_ep}" \
        "${COMMON_OPT_ARGS[@]}" \
        "${dataset_args[@]}" \
        --result_tag="${CONFIG_TAG}_${job_name}" \
        "$@"
}

add_gamma_jobs() {
    for gama in 0 0.05 0.2 0.5 1.0; do
        add_dataset_job "gamma_mnist_${MNIST_MODEL}_g${gama}" \
            mnist "${MNIST_MODEL}" 0.01 16 10 \
            --gama="${gama}" --num_steal=5 --num_img_per_client=1 \
            --agg_mode=segmented --attack_position_mode=spread

        add_dataset_job "gamma_fmnist_${FMNIST_MODEL}_g${gama}" \
            fmnist "${FMNIST_MODEL}" 0.01 16 10 \
            --gama="${gama}" --num_steal=5 --num_img_per_client=1 \
            --agg_mode=segmented --attack_position_mode=spread

        add_dataset_job "gamma_cifar_${CIFAR_MODEL}_g${gama}" \
            cifar "${CIFAR_MODEL}" 0.03 64 1 \
            --gama="${gama}" --num_steal=5 --num_img_per_client=1 \
            --agg_mode=segmented --attack_position_mode=spread
    done
}

add_num_steal_jobs() {
    for num_steal in 1 2 3 4 5 10; do
        add_dataset_job "numsteal_mnist_${MNIST_MODEL}_n${num_steal}" \
            mnist "${MNIST_MODEL}" 0.01 16 10 \
            --gama=0.5 --num_steal="${num_steal}" --num_img_per_client=1 \
            --agg_mode=segmented --attack_position_mode=spread

        add_dataset_job "numsteal_fmnist_${FMNIST_MODEL}_n${num_steal}" \
            fmnist "${FMNIST_MODEL}" 0.01 16 10 \
            --gama=0.5 --num_steal="${num_steal}" --num_img_per_client=1 \
            --agg_mode=segmented --attack_position_mode=spread

        add_dataset_job "numsteal_cifar_${CIFAR_MODEL}_n${num_steal}" \
            cifar "${CIFAR_MODEL}" 0.03 64 1 \
            --gama=0.5 --num_steal="${num_steal}" --num_img_per_client=1 \
            --agg_mode=segmented --attack_position_mode=spread
    done
}

add_position_jobs() {
    for num_img_per_client in 1 5 10 50; do
        num_steal=10
        for attack_position_mode in front spread; do
            add_dataset_job "position_mnist_${MNIST_MODEL}_n${num_steal}_img${num_img_per_client}_${attack_position_mode}" \
                mnist "${MNIST_MODEL}" 0.01 16 10 \
                --gama=0.5 --num_steal="${num_steal}" --num_img_per_client="${num_img_per_client}" \
                --agg_mode=segmented --attack_position_mode="${attack_position_mode}"

            add_dataset_job "position_fmnist_${FMNIST_MODEL}_n${num_steal}_img${num_img_per_client}_${attack_position_mode}" \
                fmnist "${FMNIST_MODEL}" 0.01 16 10 \
                --gama=0.5 --num_steal="${num_steal}" --num_img_per_client="${num_img_per_client}" \
                --agg_mode=segmented --attack_position_mode="${attack_position_mode}"

            add_dataset_job "position_cifar_${CIFAR_MODEL}_n${num_steal}_img${num_img_per_client}_${attack_position_mode}" \
                cifar "${CIFAR_MODEL}" 0.03 64 1 \
                --gama=0.5 --num_steal="${num_steal}" --num_img_per_client="${num_img_per_client}" \
                --agg_mode=segmented --attack_position_mode="${attack_position_mode}"
        done
    done
}

run_worker() {
    gpu="$1"
    gpu_slot="$2"
    worker_index="$3"
    worker_count="$4"
    total_jobs="${#JOBS_NAME[@]}"

    job_index="${worker_index}"
    while [ "${job_index}" -lt "${total_jobs}" ]; do
        job_name="${JOBS_NAME[${job_index}]}"
        job_args="${JOBS_ARGS[${job_index}]}"
        log_path="${LOG_DIR}/${job_index}_${job_name}_gpu${gpu}_slot${gpu_slot}.log"

        echo "==> [GPU ${gpu} slot ${gpu_slot}] Start job ${job_index}/${total_jobs}: ${job_name}"
        read -r -a args_array <<< "${job_args}"
        if run_fed --gpu="${gpu}" "${args_array[@]}" > "${log_path}" 2>&1; then
            echo "==> [GPU ${gpu} slot ${gpu_slot}] Done job ${job_index}: ${job_name}"
        else
            echo "!! [GPU ${gpu} slot ${gpu_slot}] Failed job ${job_index}: ${job_name}. See ${log_path}" >&2
            return 1
        fi

        job_index=$((job_index + worker_count))
    done
}

main() {
    mkdir -p "${LOG_DIR}"

    if [ "${RUN_GAMMA}" = "1" ]; then
        add_gamma_jobs
    fi
    if [ "${RUN_NUM_STEAL}" = "1" ]; then
        add_num_steal_jobs
    fi
    if [ "${RUN_POSITION}" = "1" ]; then
        add_position_jobs
    fi

    read -r -a gpus <<< "${GPU_LIST}"
    if [ "${#gpus[@]}" -eq 0 ]; then
        echo "GPU_LIST is empty." >&2
        exit 1
    fi
    if [ "${#JOBS_NAME[@]}" -eq 0 ]; then
        echo "No jobs selected." >&2
        exit 1
    fi
    if ! [[ "${JOBS_PER_GPU}" =~ ^[1-9][0-9]*$ ]]; then
        echo "JOBS_PER_GPU must be a positive integer." >&2
        exit 1
    fi

    gpu_count="${#gpus[@]}"
    worker_count=$((gpu_count * JOBS_PER_GPU))

    echo "Config: ${CONFIG_TAG}"
    echo "Total jobs: ${#JOBS_NAME[@]}"
    echo "GPUs: ${GPU_LIST}"
    echo "Jobs per GPU: ${JOBS_PER_GPU}"
    echo "Total workers: ${worker_count}"
    echo "Epochs: ${EPOCHS}"
    echo "IID: ${IID}"
    echo "Logs: ${LOG_DIR}"

    if [ "${DRY_RUN}" = "1" ]; then
        for job_index in "${!JOBS_NAME[@]}"; do
            worker_index=$((job_index % worker_count))
            gpu="${gpus[$((worker_index % gpu_count))]}"
            gpu_slot=$((worker_index / gpu_count))
            echo "DRY_RUN job ${job_index}: GPU ${gpu} slot ${gpu_slot} ${JOBS_NAME[${job_index}]} ${JOBS_ARGS[${job_index}]}"
        done
        exit 0
    fi

    pids=()
    for worker_index in $(seq 0 $((worker_count - 1))); do
        gpu="${gpus[$((worker_index % gpu_count))]}"
        gpu_slot=$((worker_index / gpu_count))
        run_worker "${gpu}" "${gpu_slot}" "${worker_index}" "${worker_count}" &
        pids+=("$!")
    done

    exit_code=0
    for pid in "${pids[@]}"; do
        if ! wait "${pid}"; then
            exit_code=1
        fi
    done

    exit "${exit_code}"
}

main
